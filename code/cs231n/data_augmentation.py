...
def random_crops(X, crop_shape):
  ...
  for n in xrange(N):
    X1 = X[n]
    # crop is in 2nd and 3rd dims
    # and can start anywhere from 0 to H-HH-1
    HHn = np.random.randint(0, H - HH)  # high exclusive
    WWn = np.random.randint(0, W - WW)  # high exclusive
    out[n] = X[n, :, HHn:HH + HHn, WWn: WW + WWn]
...
def random_contrast(X, scale=(0.8, 1.2)):
  ...
  #############################################################################
  for n in xrange(N):
    contrast = np.random.random() * (scale[1] - scale[0]) + scale[0]
    X1 = X[n] * contrast
    out[n] = X1
  #############################################################################
  #                           END OF YOUR CODE                                #
...
def random_tint(X, scale=(-10, 10)):
  ...
  #############################################################################
  for n in xrange(N):
      tint = np.random.random((3, 1, 1)) * (scale[1] - scale[0]) + scale[0]
      X1 = X[n] + tint
      out[n] = X1
  #############################################################################
  #                           END OF YOUR CODE                                #
