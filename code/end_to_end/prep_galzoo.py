...
def make_train_and_val_listfiles(frac_val=0.2):
    ...
    filenames = glob('caffe_data/galzoo_processed/*.jpg')
    np.random.shuffle(filenames)
    label_dict = get_kmeans_labels()
    f_train = open('caffe_data/galzoo_train_listfile','w')
    f_val = open('caffe_data/galzoo_val_listfile','w')
...
def get_kmeans_labels(n_clusters=20):
    ...
    f = open('galzoo/training_solutions_rev1.csv')
    f.next()
    ids=[]
    scores = []
    for line in f:
...
def preprocess_training_images(savedir='caffe_data/galzoo_processed/'):
    import os
    from scipy.misc import imsave
    from glob import glob
    filenames = glob('galzoo/images_training_rev1/*.jpg')
